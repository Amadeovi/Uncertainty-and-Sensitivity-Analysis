...
## Not run:
## Simulate data from this GamVine:
sim <- gamVineSimulate(n, GVC)

# then we are going to fit this data:
...
# warning does not lead to good results:
# Now we will study the logLikelihood our non-simplified model evaluated at our sim. data
# for the 1st copula: Clayton we have:
#term1<-sum(log(BiCopPDF(X1, X2, family=3, par=2, par2=0, obj=NULL, check.pars=TRUE)))
# for the 2nd copula: Gumbell we have:
#term2<-sum(log(BiCopPDF(X2, X3, family=4, par=1.5, par2=0, obj=NULL, check.pars=TRUE)))
...
#term3<-sum(log(BiCopPDF(X1_2 , X3_2, family=1, par=rho, par2=0, obj=NULL, check.pars=TRUE)))
